...
export default function CountdownPosition() {
	const center = useCenterStore()
	const { cardStyles } = useConfigStore()
	const styles = cardStyles.countdownPosition
	...
	const y =
		styles.offsetY !== null
			...
			: center.y + hiCardStyles.height / 2 + CARD_SPACING + socialButtonsStyles.height + CARD_SPACING + musicCardStyles.height + CARD_SPACING + 80

	return (
		<HomeDraggableLayer cardKey='countdownPosition' x={x} y={y} width={styles.width} height={styles.height}>
			<motion.div className='absolute max-sm:static' initial={{ left: x, top: y }} animate={{ left: x, top: y }}>
				<CountdownButton delay={cardStyles.shareCard.order * ANIMATION_DELAY * 1000} />
			</motion.div>
		</HomeDraggableLayer>
	)
}
